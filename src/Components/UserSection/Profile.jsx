...
const Profile = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch(`${ServerUrl}/api/user/updateProfile`, {
                ...
                body: JSON.stringify({ id: userId, image: base64Image,password:Password }),
            });
    ...
    return (
        <div className='w-full gap-4 flex flex-col justify-center items-center pt-4 overflow-y-scroll'>
            <form className='flex flex-col justify-center items-center' onSubmit={handleSubmit}>
                ...
                <div className='flex flex-col gap-2 mt-4 w-full'>
                    <input type="name" placeholder="Name" className="input p-2 rounded-lg" value={name} disabled />
                    <input type="email" placeholder="Email" className="input p-2 rounded-lg" value={email} disabled />
                </div>
                <p className='text-l p-2 cursor-pointer hover:bg-blue-500 hover:text-white m-2 rounded' onClick={() => setChangePassword(!ChangePassword)}>{ChangePassword ? "Cancel" : "Change Password"}</p>
                {
                    ChangePassword &&
                        <div className='flex flex-col gap-2 mb-2'>
                            <input type="password" placeholder="Enter New Password" className="input p-2 rounded-lg" value={Password} onChange={(e) => setPassword(e.target.value)} />
                            <input type="password" placeholder="Confirm Password" className="input p-2 rounded-lg" value={ConfirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
                        </div> 
                }
                <input type="submit" className='bg-blue-500 hover:bg-blue-700 text-white p-2 mt-2 rounded' value="Save changes" />
            </form>
        </div>
    );
}
