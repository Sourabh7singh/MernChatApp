...
const SearchUserMenu = (props) => {
    ...
    return (
        <div className="z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-[250px] dark:bg-gray-700 dark:divide-gray-600 absolute top-10 right-0">
            {
                Section === 'Chats' &&
                <div className='SearchUser'>
                    <div className='flex justify-between m-2'>
                        <label htmlFor="user" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Search For a user</label>
                    ...
                    <div className='m-2'>
                        <input type="text" name="email" id="user" className="bg-gray-50 border w-full border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Enter Name or Username" required value={search} onChange={(e) => setSearch(e.target.value)} />
                    </div>
            ...
            {
                Section === 'Groups' && <div className="pt-2 text-sm text-gray-700 dark:text-gray-200 flex justify-between items-center">
                    <h6 className='text-center text-white pl-2'>Create Group</h6>
                    <div className="done cursor-pointer pr-2" title='done?'>
            ...
            }
            {
                ...
                <div className='relative p-2'>
                    <div className='text-center font-thin text-lg p-2 m-1'>
                        <input type="text" placeholder='Enter the name of the group' id='group' className='bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white w-full' value={groupName} onChange={(e) => { setGroupName(e.target.value) }} />
                    </div>
                    <div className='SearchUser font-thin m-1 p-2'>
                        <div className='flex justify-between'>
                            <label htmlFor="Groupuser" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Search a users to add</label>
                        ...
                        {SelectedGroupUsers.length > 0 && <div className="selected-users">
                            ...
                            {SelectedGroupUsers && SelectedGroupUsers.map((user, index) => {
                                return <span className="selected-user text-white inline-block justify-evenly items-center text-center font-thin text-sm p-2 m-1 cursor-pointer rounded-2xl hover:bg-slate-600 bg-slate-800" key={index}>
                                    {user.name}
                                    <svg className="ml-1 inline text-gray-800 dark:text-white cursor-pointer w-[20px]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" onClick={() => setSelectedGroupUsers(SelectedGroupUsers.filter((item) => item !== user))}>
